                ...
                <div className="container">
                    <div className="flex flex-col space-y-4 py-5">
                        <div className="flex items-center justify-center space-x-8">
                            {menu.map((item, index) => (
                                <Link key={index} href={item.href} openNewTab={true} className="animated-underline inline-flex items-center justify-center text-xs font-semibold">
                                    {item.name}
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center justify-center space-x-8">
                            {socialMedia.map(({ icon: Icon, ...item }, index) => (
                                <div key={index} className="flex items-center space-x-2">
                                    <Icon className="h-4 w-4 text-sky-700 dark:text-sky-600" /> <Link href={item.href} openNewTab={true} className="animated-underline text-xs font-semibold">
