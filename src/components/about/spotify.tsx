...
export const Spotify = () => {
    const { data } = useSWR<NowPlayingType | undefined>('/api/spotify');

    const changeMillisecondsToMinutes = (ms = 0) => {
    ...
    const getPersentase = (progress = 0, duration = 0) => {
        let result: number = 0;
    ...
    return (
        <Section id="now-playing" title="Now Playing on Spotify" description="Listen to my music on Spotify" className="pt-4">
            <div className="pt-4">
                {data && data.isPlaying ? (
                    <div className="space-y-6 rounded-xl border border-slate-200 bg-white p-4 pb-6 dark:border-transparent dark:bg-slate-800 sm:space-y-8 sm:p-10 sm:pb-8 lg:space-y-6 lg:p-6 xl:space-y-8 xl:p-10 xl:pb-8">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="min-w-0 flex-auto space-y-1 font-semibold">
                                <Link href={data.songUrl} openNewTab={true} className="text-sm leading-6 text-cyan-500 dark:text-cyan-400">
                                    Open on Spotify
                                </Link>
                    ...
                    <div className="block w-full border border-dashed border-slate-800 py-2 px-4 text-sm">
