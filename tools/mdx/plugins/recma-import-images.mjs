...
export default function recmaImportImages({ exportName = 'default', property } = {}) {
    return (tree) => {
        slugify.reset();
        ...
        visit(tree, (node) => {
            ...
            if (node.callee.name !== '_jsx' && node.callee.name !== '_jsxDEV' && node.callee.name !== '_jsxs') return;
            ...
            let name = images[path] ?? `_${slugify(srcProp.value.value).replace(/-/g, '_')}$recmaImportImages`;
            images[path] = name;
